...
export default async function AdminManufacturersPage() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(errorBody?.error || 'Failed to fetch manufacturers')
    }
                  ...
                  {manufacturers.map((manufacturer: any) => (
                    <tr key={manufacturer.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                        ...
                        <form action={`/api/admin/manufacturers/${manufacturer.id}/delete`} method="POST" style={{ display: 'inline' }}>

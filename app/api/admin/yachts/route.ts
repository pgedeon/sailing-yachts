...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, manufacturer, lengthOverall, beam, draft, displacement, year, imageUrl } = body;
    if (!name || !manufacturer) {
      return NextResponse.json({ error: "Name and manufacturer are required" }, { status: 400 });
    }
    const [inserted] = await db.insert(yachts).values({
      id: randomUUID(), name, manufacturer, lengthOverall, beam, draft, displacement, year, imageUrl,
    }).returning();

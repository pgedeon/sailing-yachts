  ...
  try {
    ...
    const result = await pool.query(`
      ...
      SELECT 
        ...
        COALESCE(mac.media_count, 0) as media_asset_count,
    ...
    const totalImages = yachts.reduce((sum, y) => sum + y.image_count, 0)
    const totalMediaAssets = yachts.reduce((sum, y) => sum + y.media_asset_count, 0)
